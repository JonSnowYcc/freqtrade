            ...
            for csv_file in csv_files:
                with zip_ref.open(csv_file) as csv_content:
                    ...
                    for line in lines:
                        ...
                        # 简单分割，只取前6个字段
                        parts = line.split(',', 5)  # 最多分割5次，得到6个部分
                        
                        if len(parts) < 6:
                            continue
                        
                        # 检查是否是标题行
                        if parts[0] in ['open_time', 'Open time', 'timestamp', 'Timestamp']:
                            continue
...
class SimpleBinanceConverter:
    def __init__(self, input_dir: str, output_dir: str, max_workers: int = None):
        ...
        if max_workers is None:
            self.max_workers = min(mp.cpu_count(), 4)  # 减少进程数避免内存问题
        else:
...
def main():
    parser = argparse.ArgumentParser(description='简单高效的币安数据转换')
    ...
    # 创建转换器
    converter = SimpleBinanceConverter(args.input, args.output, args.workers)
