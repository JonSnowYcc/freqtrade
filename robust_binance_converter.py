        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            for csv_file in csv_files:
                with zip_ref.open(csv_file) as csv_content:
                    ...
                    if content is None:
                        logger.warning(f"无法解码文件: {csv_file}")
                        continue
                    ...
                    for row_num, row in enumerate(csv_reader):
                        ...
                        # 跳过标题行
                        if row[0] in ['open_time', 'Open time', 'timestamp', 'Timestamp']:
                            continue
                        ...
                        try:
                            ...
                            timestamp = int(float(row[0])) // 1000  # 转换为秒
                            ...
                            # 跳过无效的数据行
                            if row_num < 10:  # 只记录前10行的错误
                                logger.debug(f"跳过无效行 {row_num}: {e}")
                            continue
            ...
            if output_path.exists():
                try:
                    ...
                    logger.warning(f"合并数据时出错，覆盖文件: {e}")
...
class RobustBinanceConverter:
    def __init__(self, input_dir: str, output_dir: str, max_workers: int = None):
        ...
        if max_workers is None:
            self.max_workers = min(mp.cpu_count(), 6)  # 最多使用6个进程
        else:
...
def main():
    parser = argparse.ArgumentParser(description='健壮的币安数据转换为Freqtrade格式')
    ...
    # 创建健壮转换器
    converter = RobustBinanceConverter(args.input, args.output, args.workers)
